...
// ══════════════════════════════════════════════════════════
// Credenciais fixas para que os visitantes não precisem configurar
let SB_URL = 'https://qttqogvwcapksldsszxa.supabase.co';
let SB_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF0dHFvZ3Z3Y2Fwa3NsZHNzenhhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQzMTMxMzksImV4cCI6MjA4OTg4OTEzOX0.Gn1I4qTCxvuD1L6Ll_4anTEfkIInfxc2bHJwe-BxZYw';
let ADMIN_PASS = '';
let isAdmin = false;
let fabOpen = false;
...
-- Configuração do Storage (Bucket e Políticas)
insert into storage.buckets (id, name, public) values ('memorial-media', 'memorial-media', true) on conflict do nothing;

create policy "Leitura pública storage" on storage.objects for select using ( bucket_id = 'memorial-media' );
create policy "Escrita pública storage" on storage.objects for insert with check ( bucket_id = 'memorial-media' );
create policy "Atualização pública storage" on storage.objects for update using ( bucket_id = 'memorial-media' );
create policy "Deleção pública storage" on storage.objects for delete using ( bucket_id = 'memorial-media' );
`.trim();
  ...
  try {
    ...
    document.getElementById('loadingScreen').innerHTML = `
      <div style="text-align:center;padding:40px;color:var(--text-soft)">
        <p style="font-family:'Playfair Display',serif;font-size:24px;font-style:italic;margin-bottom:16px;color:var(--deep-gold)">
          Erro ao conectar ao banco de dados
        </p>
        <p style="font-size:14px;margin-bottom:24px">${e.message.substring(0,120)}</p>
...
window.addEventListener('DOMContentLoaded', () => {
  // Se as credenciais estiverem fixas no código, usa elas e pula o setup
  if (SB_URL && SB_KEY) {
    document.getElementById('setupScreen').style.display = 'none';
    startApp();
  } else {
    // Fallback para o localStorage (caso as credenciais sejam removidas do código)
    const url = localStorage.getItem('sb_url');
    const key = localStorage.getItem('sb_key');
    if (url && key) {
      SB_URL = url; SB_KEY = key;
